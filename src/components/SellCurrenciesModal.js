...
class SellCurrenciesModal extends Component {
	...
	componentDidMount() {
		...
		const initialCoin = (this.state && this.state.sell) || 'bitcoin';
	...
	onChangeSellCurrency(e) {
		...
		this.setState({ sell: selectedCoin });
	...
	sellCurrencies(e) {
		...
		const chosenCurrencyName = this.state.sell || 'Bitcoin';
		...
		fetch(`${process.env.REACT_APP_BACKEND_URL}/users/${this.props.userIdSelling}/sell`, {
			...
			body: JSON.stringify({
				...
				currency_to_sell: chosenCurrencyName,
			...
			<div className="modal-backdrop-wrapper" style={{
				...
				<div className="modal-dialog m-0" role="document" onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: '450px', zIndex: 3001 }}>
					<div className="modal-content border-0 shadow-lg" style={{ borderRadius: '16px', overflow: 'hidden', backgroundColor: '#ffffff' }}>
						...
						{/* HEADER BLOCK */}
						<div className="modal-header bg-danger text-white p-3 d-flex justify-content-between align-items-center" style={{ borderBottom: 'none' }}>
							<h5 className="modal-title m-0 font-weight-bold" style={{ fontSize: '1.2rem', color: '#ffffff' }}>Sell Game Currencies</h5>
						...
						<div className="modal-body p-4 bg-white" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
							<form onSubmit={this.sellCurrencies}>
								...
								<div className="form-group mb-3">
									<label className="small font-weight-bold text-muted mb-1">Select Currency Asset to Liquidate:</label>
									<select 
										onChange={(e) => {
											this.setState({ sell: e.target.value });
											if(this.fetchLiveSpotPrice) this.fetchLiveSpotPrice(e.target.value);
										}} 
										value={this.state.sell || 'Bitcoin'} 
								...
								<div className="p-3 mb-3 border rounded text-center bg-light">
									...
									<span className="h4 text-danger font-weight-bold">
								...
								<div className="form-group mb-4">
									<label className="small font-weight-bold text-muted mb-1">Number of Units to Sell:</label>
									<input 
										ref="numberOfUnits" 
										type="number" 
										step="any"
								...
								<div className="actions-button-deck mt-2">
									<button type="submit" className="btn btn-danger w-100 font-weight-bold py-2 mb-2" style={{ borderRadius: '8px', fontSize: '16px' }}>
										Confirm Sell Order
									</button>
